...
1326 D1 - Prefix-Suffix Palindrome (Easy version).cpp GNU C++17 Accepted
#include <bits/stdc++.h>
...
void test(){
    ...
    string remains1 = "", remains2 = "";
    string s11 = s.substr(i,j-i+1);
    for(int i = 1;i<=s11.size();i++){
        string check = s11.substr(0,i);
        if(isPalindrome(check)){
            remains1 = check;
        }
    }
    for(int i = 2;i<=s11.size();i++){
        string check = s11.substr(s11.size()-i,i);
        if(isPalindrome(check)){
            remains2 = check;
        }
    }
    reverse(all(s2));
    if(remains1.size()>remains2.size()){
        s1 += remains1;
    }
    else
        s2 = remains2 + s2;
